...
async function openDetail(id: string) {
  ...
  loadingDetail.value = true
  try {
    // 仍用 load('detail') 统一模式；UI 层用 Descriptions 展示
...
<template>
  <div class="demo">
    <p class="hint">
      弹窗场景：列表只调 open*。create/edit 渲染表单；detail 渲染文字 Descriptions，不是 disabled 输入框。
    </p>
    ...
    <pre class="log">{{ last || '提交成功后显示' }}</pre>
  </div>
</template>
