...
export type FormResult<T> =
  | { ok: true, values: T, errors?: undefined }
...
export type RulesSource<T extends Record<string, unknown>> =
  | FormRulesInput
...
export interface CreateFormOptions<T extends Record<string, unknown>> {
  ...
  onSubmit?: (values: T, ctx: { form: FormApi<T> }) => void | Promise<void>
  onInvalid?: (errors: FormErrors, ctx: { form: FormApi<T> }) => void
}
...
export interface FormApi<T extends Record<string, unknown>> {
  ...
  fieldArray: <TItem extends Record<string, unknown> = Record<string, unknown>>(
    path: FieldPath,
